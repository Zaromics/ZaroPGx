...
async def get_pharmcat_summary_endpoint(run_id: str, db: Session = Depends(get_db)):
    """
...
async def get_genes(run_id: str, db: Session = Depends(get_db)):
    """
...
async def get_diplotypes(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error getting diplotypes: {str(e)}"
        )
...
async def get_actionable_findings(run_id: str, db: Session = Depends(get_db)):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error getting actionable findings: {str(e)}"
        )
...
async def list_pharmcat_runs(
    limit: int = Query(
        10, ge=1, le=100, description="Maximum number of runs to return"
    ),
...
async def delete_pharmcat_run(run_id: str, db: Session = Depends(get_db)):
    """
...
async def health_check():
    ...
    return {"status": "healthy", "service": "pharmcat-api", "version": "0.3.0"}
