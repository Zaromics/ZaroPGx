...
class JobBase(BaseModel):
    ...
    progress: int = Field(
        ..., ge=0, le=100, description="Progress percentage from 0 to 100"
    )
...
class JobCreate(BaseModel):
    ...
    initial_stage: JobStage = Field(
        JobStage.UPLOAD, description="Starting stage for the job"
    )
    job_metadata: Dict[str, Any] = Field(
        default_factory=dict, description="Initial metadata for the job"
    )
...
class JobUpdate(BaseModel):
    ...
    progress: Optional[int] = Field(
        None, ge=0, le=100, description="New progress percentage"
    )
...
class JobStageResponse(BaseModel):
    ...
    completed_at: Optional[datetime] = Field(
        None, description="When the stage completed"
    )
    duration_ms: Optional[int] = Field(
        None, ge=0, description="Stage duration in milliseconds"
    )
