...
def get_override_file_path() -> Optional[str]:
    ...
    override_enabled = os.getenv("OUTSIDECALLSOVERRIDE", "false").lower() in {"true", "1", "yes", "on"}
    ...
    possible_paths = [
        ...
        os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), DEFAULT_OVERRIDE_PATH)
    ]
    ...
    logger.warning(f"Outside calls override is enabled but no override file found at any of these locations: {possible_paths}")
    return None
...
def is_override_enabled() -> bool:
    ...
    """
    return os.getenv("OUTSIDECALLSOVERRIDE", "false").lower() in {"true", "1", "yes", "on"}
...
def validate_override_file(file_path: str) -> bool:
    ...
    try:
        ...
        if len(lines) < 1:
            logger.error(f"Override file {file_path} is empty or contains only comments")
            return False
        ...
        logger.info(f"Override file {file_path} validation passed with {len(valid_genes)} gene entries: {valid_genes}")
        return True
...
def get_override_file_content(file_path: str) -> Optional[str]:
    ...
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            content = f.read()
        logger.info(f"Successfully read override file content ({len(content)} characters)")
        return content
    except Exception as e:
