        ...
        with zipfile.ZipFile(zip_path, "r") as zip_ref:
            ...
            for info in zip_ref.infolist():
                if info.file_size > 50_000_000:  # 50MB
                    ...
                    logger.info(
                        f"Large file in ZIP: {info.filename} ({info.file_size / 1_000_000:.2f} MB)"
                    )
                    print(
                        f"[ZIP] Large file detected: {info.filename} ({info.file_size / 1_000_000:.2f} MB)"
                    )
            ...
            vcf_files = [
                f for f in file_list if f.lower().endswith((".vcf", ".vcf.gz"))
            ]
            ...
            if bam_files:
                ...
                bam_paths = [
                    (f, os.path.getsize(os.path.join(extract_dir, f)))
                    for f in bam_files
                ...
                if bam_paths:
                    ...
                    logger.info(
                        f"Selected largest BAM file: {largest_bam} ({bam_paths[0][1] / 1_000_000:.2f} MB)"
                    )
            ...
            if not genomic_files and vcf_files:
                ...
                if vcf_paths:
                    ...
                    logger.info(
                        f"Selected largest VCF file: {largest_vcf} ({vcf_paths[0][1] / 1_000_000:.2f} MB)"
                    )
            ...
            if not genomic_files:
                ...
                if genomic_files:
                    logger.info(
                        f"Using first available genomic file: {genomic_files[0]}"
                    )
...
async def create_workflow_for_job(
    job_id: str, file_type: str, sample_id: str, reference_genome: str, db: Session
) -> str:
    ...
    try:
        async with WorkflowClient(
            ...
            if status == "running":
                ...
                await client.update_step_status(
        ...
        logger.error(
            f"Failed to update workflow progress for {workflow_id}/{step_name}: {e}"
        )


def update_workflow_progress_sync(
    workflow_id: str,
    step_name: str,
    status: str,
    message: str = None,
...
async def get_cleanup_status():
    ...
    try:
        ...
        logger.info(
            f"Test BAM processing: File saved to {file_path}, size: {file_size}, type: {file_type}"
        )
        print(
            f"[TEST] BAM file saved: {file_path}, size: {file_size}, type: {file_type}"
        )
